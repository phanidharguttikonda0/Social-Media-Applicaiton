...
export async function changePassword(req:AuthenticatedRequest, res:Response){
    try{
        ...
        if(newPassoword && newPassoword.length >= 8){
        ...
        }else  res.json({data: false, message: "failed due to no empty password"})
...
export async function setAccountType(req:AuthenticatedRequest, res:Response){
    try{
        ...
        if (!user) {
            ...
            return res.json({data: false, message: "user not found"});
        }
        ...
        return res.json({data: true, message:"done successfully"})
